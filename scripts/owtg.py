...
#Gets lines from file and returns them as an array; does not keep trailing newlines
...
#Returns and array of OWTGSensor objects
def getSensors():
...
#Returns an array of dictionaries with 'param' and 'value'
def datGetDictList():
...
#Returns the value of a parameter in the .dat file
def datGet(param):
...
#Sets the value of a parameter in the .dat file
...
#Sends and alert email to the email address specified in the web interface/.dat file
def alertmail(subject,body):
    ...
    smtp = smtplib.SMTP('localhost') #Using localhost as an SMTP server
